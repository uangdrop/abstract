...
if [ -d "abstract" ]; then
  rm -rf abstract
  show "Removed existing abstract folder." "done"
fi
...
show "Setting up abstract and OpenZeppelin..." "progress"
mkdir -p abstract && cd abstract
...
EOF
  ...
  for ((i=0; i<NUM_ADDRESSES; i++)); do
      ...
      SEND_CMD="cast send \"$CONTRACT_ADDRESS\" \"transfer(address,uint256)\" \"$RANDOM_ADDRESS\" \"$AMOUNT_WEI\" --rpc-url https://api.testnet.abs.xyz --private-key \"$YOUR_PRIVATE_KEY\""
  ...
  show "https://explorer.testnet.abs.xyz/address/$YOUR_ADDRESS#tokentxns" "done"
...
EOF
  ...
  DEPLOY_CMD="forge create winsnip/MyNFT.sol:MyNFT --rpc-url https://api.testnet.abs.xyz --private-key $YOUR_PRIVATE_KEY --constructor-args $OWNER_ADDRESS"
  eval $DEPLOY_CMD
  ...
  read -p "How many NFTs to mint: " NUM_NFTS
    ...
    for ((i=0; i<OWNER_MINT_COUNT; i++)); do
        ...
        MINT_CMD="cast send \"$NFT_CONTRACT_ADDRESS\" \"safeMint(address,string)\" \"$OWNER_ADDRESS\" \"$TOKEN_URI\" --rpc-url https://api.testnet.abs.xyz --private-key \"$YOUR_PRIVATE_KEY\""
        eval $MINT_CMD
    done
    ...
    for ((i=0; i<RANDOM_MINT_COUNT; i++)); do
        ...
        MINT_CMD="cast send \"$NFT_CONTRACT_ADDRESS\" \"safeMint(address,string)\" \"$RANDOM_ADDRESS\" \"$TOKEN_URI\" --rpc-url https://api.testnet.abs.xyz --private-key \"$YOUR_PRIVATE_KEY\""
        eval $MINT_CMD
    done
  show "NFT contract can be viewed at: https://explorer.testnet.abs.xyz/$NFT_CONTRACT_ADDRESS" "done"
